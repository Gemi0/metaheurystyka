...
public class SingleGeneticScout extends SingleScout{

    private final Random rnd;
    private final double probability;

    public SingleGeneticScout(SingleMeadow singleMeadow, TSPData data, int maxRetries, double probability) {
    ...
    public void sendBees() {
        ...
        for (int i = 0; i <singleMeadow.getFlowers().size(); i++) {
            flower = singleMeadow.getFlowers().get(i);
            if (flower.getRetriesCounter() > maxRetries && flower.getPermutationValue() > singleMeadow.getBestFlower().getPermutationValue()) {
                if (flowerToBreedIndex == -1) {
                    ...
                    if (rnd.nextDouble() <= probability) {
                        PMXCrossbreeding(flower, singleMeadow.getFlowers().get(flowerToBreedIndex));
                        if (flower.getPermutationValue() < singleMeadow.getBestFlower().getPermutationValue()) {
                            singleMeadow.setBestFlower(flower);
                        }
                        if (singleMeadow.getFlowers().get(flowerToBreedIndex).getPermutationValue() < singleMeadow.getBestFlower().getPermutationValue()) {
                            singleMeadow.setBestFlower(flower);
                        }
